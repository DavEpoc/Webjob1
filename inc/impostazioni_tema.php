...
function pagina_impostazioni_tema_cb() {
    $pagina_impostazioni = add_theme_page(
            ...
            'impostazioni-tema', // Slug della pagina, utilizziamo __FILE__ per dare uno slug unico
...
function validazione_impostazioni_tema($input) {
    // echo "<pre>"; paint_r( $_FILES ); echo "</pre>";
    foreach ($_FILES as $image) {
        ...
        if ($image['size']) {
            if (preg_match('/(jpg|jpeg|png|gif)$/i', $image['type'])) {
                ...
                wpam_die('Nessuna immagine e&amp;grave; stata caricata');
            }
        } else {
